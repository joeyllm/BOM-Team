...
Why are you building a model that is not physics-informed?(in other words, Given that atmospheric evolution is strongly governed by physical laws, why does the proposed model not incorporate any physics-informed constraints? Is this an intentional design choice?) How can you train this model to learn real atmosphere climate change rather than just mimicing ACCESS-C's own time-stepping?

~~What is your story? Are you building a forecast model or representation?~~

Since we are not using accum_rain variable at this stage, will we add other variables such as rain rate?

Need a method to deal with errors snowballing in the rollout. Error accumulates during autoregressie rollout. e.g.scheduled sampling, noise injection, or curriculum-style training on rollout length

The "modularity" claim doesn't have an experiment. We need to evaluate the performance of the original "modularity" model and some model with a changed module(such as change an encoder).